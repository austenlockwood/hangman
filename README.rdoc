...
User has 7 turns to guess the word, stored at the start of the game in a variable called "turns" with a value of "7."

User guesses are stored in an initially empty array called "guesses."

---

User types "ruby fuck_you_hangman.rb" in the console to start the program.
...
"stab" travels through an if/else statement to determine if it is in "secret_word." To do this, the .include? method is called on secret_word with "stab" as the parameter.

If "stab" is in "secret_word," the console displays a message telling the user. If "stab" is not in "secret_word," the console displays a message telling the user; the value of "turn" is reduced by one as well.  In both instances, "stab" is pushed into "guesses."  The if/else statement ends here and the rest of the loop is executed.
...
Turns left are displayed with a simple string interpolation of the variable turns.
...
If the output does not include any unguessed letters, the console displays a win message; if it does, the console displays a lose message.
